...
# MODEL: Meriden ~ Meriden (lags<=6) + Bradley (current + lags<=6)
...
# Bradley station file + NARR (USW00014740)
path_B   <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/concatenated_files/merged_imputed_final/USW00014740_merged_imputed_2004_2024.csv"
narr_B   <- "/home/fbs24003/rlibs/Precip_project/narr_reanalysis_data/USW00014740.csv"

outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_meriden_bradley"
...
datB <- datB[DATE >= as.POSIXct("2004-01-01 00:00:00", tz = "UTC")]
...
DT[, `:=`(
  alpha_tilde_B = 0,
  S_h_B     = numify(HourlyWindSpeed_B),
  theta_h_B = numify(HourlyWindDirection_B)
)]
DT[, rad_B := ((alpha_tilde_B + 630 - theta_h_B) %% 360) * pi/180]
DT[, `:=`(
  WindE_B = S_h_B * cos(rad_B),
  WindN_B = S_h_B * sin(rad_B)
)]
...
setnames(narr_h_B, c("u","v","dew","rh","slp"),
...
DT[, `:=`(
  ...
  dew_WindE_B = dew_B * WindE_B,
  dew_WindN_B = dew_B * WindN_B
)]

# --------------------------------
# Lags (Meriden + Bradley)
# --------------------------------
for (i in 1:L) {
  # Meriden lags
...
# log1p on rainfall lags (both stations) + OPTIONAL current Bradley rain
for (i in 1:L) {
  DT[, paste0("rain_M_lag", i) := log1p(get(paste0("rain_M_lag", i)))]
  DT[, paste0("rain_B_lag", i) := log1p(get(paste0("rain_B_lag", i)))]
}
DT[, rain_B_log1p := log1p(rain_B)]  # current-time Bradley rain (log1p)
...
rhs_mu_gb <- paste(
  c(
    ...
    # --- current-time Bradley rain (log1p) + met terms ---
    ...
    bbs_term("WindE_B"),
    bbs_term("WindN_B"),
    bbs_term("dew_WindE_B"),
    bbs_term("dew_WindN_B"),
    ...
    # --- Bradley lag terms ---
    vapply(paste0("rain_B_lag", 1:1), bols_term_rain, character(1)),
    vapply(paste0("rain_B_lag", 2:L), bbs_term, character(1)),
    vapply(paste0("RH_B_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindN_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindE_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindN_B_lag", 1:L), bbs_term, character(1))
  ),
  collapse = " + "
)
...
rhs_sigma_gb <- paste(
  c(
    ...
    # --- Bradley lagged terms ---
    vapply(paste0("rain_B_lag", 1:L_sigma), bbs_term, character(1)),
    vapply(paste0("RH_B_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_B_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_B_lag", 1:L), bbs_term, character(1)),
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_meriden_bradley.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Meriden with Bradley exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_meriden_bradley.pdf"), width = 12, height = 6)
print(p)
dev.off()
