...
# MODEL: Meriden ~ Meriden (lags<=6) + Sikorsky (current + lags<=6)
...
# Sikorsky station file + NARR (USW00094702)
path_S   <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/concatenated_files/merged_imputed_final/USW00094702_merged_imputed_2004_2024.csv"
narr_S   <- "/home/fbs24003/rlibs/Precip_project/narr_reanalysis_data/USW00094702.csv"

outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_meriden_sikorsky"
...
datS <- fread(path_S, select = cols_needed)

datM[, DATE := as.POSIXct(DATE, tz = "UTC")]
datS[, DATE := as.POSIXct(DATE, tz = "UTC")]

datM <- datM[DATE >= as.POSIXct("2004-01-01 00:00:00", tz = "UTC")]
datS <- datS[DATE >= as.POSIXct("2004-01-01 00:00:00", tz = "UTC")]
...
# Sikorsky core vars
# -----------------------------
DT[, rain_S := numify(HourlyPrecipitation_S)]
DT[, RH_S   := numify(HourlyRelativeHumidity_S)]
DT[, dew_S  := numify(HourlyDewPointTemperature_S)]
DT[, slp_S  := numify(HourlySeaLevelPressure_S)]

DT[, `:=`(
  alpha_tilde_S = 0,
  S_h_S     = numify(HourlyWindSpeed_S),
  theta_h_S = numify(HourlyWindDirection_S)
)]
DT[, rad_S := ((alpha_tilde_S + 630 - theta_h_S) %% 360) * pi/180]
DT[, `:=`(
  WindE_S = S_h_S * cos(rad_S),
  WindN_S = S_h_S * sin(rad_S)
)]
...
# --------------------------------
# NARR imputation (Sikorsky)
# --------------------------------
narr_h_S <- narr_to_hourly(narr_S)
setnames(narr_h_S, c("u","v","dew","rh","slp"),
         c("u_S","v_S","dew_S_narr","rh_S_narr","slp_S_narr"))

DT <- DT[narr_h_S, on = "DATE"]

DT[is.na(WindE_S) & !is.na(u_S),        WindE_S := u_S]
DT[is.na(WindN_S) & !is.na(v_S),        WindN_S := v_S]
DT[is.na(dew_S)  & !is.na(dew_S_narr),  dew_S  := dew_S_narr]
DT[is.na(RH_S)   & !is.na(rh_S_narr),   RH_S   := rh_S_narr]
DT[is.na(slp_S)  & !is.na(slp_S_narr),  slp_S  := slp_S_narr]

DT[, c("u_S","v_S","dew_S_narr","rh_S_narr","slp_S_narr") := NULL]
...
DT[, `:=`(
  ...
  dew_WindE_S = dew_S * WindE_S,
  dew_WindN_S = dew_S * WindN_S
)]

# --------------------------------
# Lags (Meriden + Sikorsky)
# --------------------------------
for (i in 1:L) {
  # Meriden lags
  ...
  # Sikorsky lags
  DT[, paste0("rain_S_lag",    i) := shift(rain_S,    n = i, type = "lag")]
  DT[, paste0("RH_S_lag",      i) := shift(RH_S,      n = i, type = "lag")]
  DT[, paste0("dew_S_lag",     i) := shift(dew_S,     n = i, type = "lag")]
  DT[, paste0("slp_S_lag",     i) := shift(slp_S,     n = i, type = "lag")]
  DT[, paste0("WindE_S_lag",   i) := shift(WindE_S,   n = i, type = "lag")]
  DT[, paste0("WindN_S_lag",   i) := shift(WindN_S,   n = i, type = "lag")]
  DT[, paste0("dew_WindE_S_lag", i) := shift(dew_WindE_S, n = i, type = "lag")]
  DT[, paste0("dew_WindN_S_lag", i) := shift(dew_WindN_S, n = i, type = "lag")]
}

# log1p on rainfall lags (both stations) + OPTIONAL current Sikorsky rain
for (i in 1:L) {
  DT[, paste0("rain_M_lag", i) := log1p(get(paste0("rain_M_lag", i)))]
  DT[, paste0("rain_S_lag", i) := log1p(get(paste0("rain_S_lag", i)))]
}
DT[, rain_S_log1p := log1p(rain_S)]  # current-time Sikorsky rain (log1p)
...
vars <- c(
  ...
  # Sikorsky (current + lags)
  "rain_S_log1p",
  paste0("rain_S_lag", 1:L),
  "RH_S", paste0("RH_S_lag", 1:L),
  "dew_S", paste0("dew_S_lag", 1:L),
  "slp_S", paste0("slp_S_lag", 1:L),
  "WindE_S", paste0("WindE_S_lag", 1:L),
  "WindN_S", paste0("WindN_S_lag", 1:L),
  "dew_WindE_S", paste0("dew_WindE_S_lag", 1:L),
  "dew_WindN_S", paste0("dew_WindN_S_lag", 1:L)
)
...
rhs_mu_gb <- paste(
  c(
    ...
    # --- current-time Sikorsky rain (log1p) + met terms ---
    bbs_term("rain_S_log1p"),
    bbs_term("RH_S"),
    bbs_term("dew_S"),
    bbs_term("slp_S"),
    bbs_term("WindE_S"),
    bbs_term("WindN_S"),
    bbs_term("dew_WindE_S"),
    bbs_term("dew_WindN_S"),
    ...
    # --- Sikorsky lag terms ---
    vapply(paste0("rain_S_lag", 1:1), bols_term_rain, character(1)),
    vapply(paste0("rain_S_lag", 2:L), bbs_term, character(1)),
    vapply(paste0("RH_S_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindN_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindE_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindN_S_lag", 1:L), bbs_term, character(1))
  ),
  collapse = " + "
)
...
rhs_sigma_gb <- paste(
  c(
    ...
    # --- Sikorsky lagged terms ---
    vapply(paste0("rain_S_lag", 1:L_sigma), bbs_term, character(1)),
    vapply(paste0("RH_S_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_S_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_S_lag", 1:L), bbs_term, character(1)),
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_meriden_sikorsky.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Meriden with Sikorsky exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_meriden_sikorsky.pdf"), width = 12, height = 6)
print(p)
dev.off()
