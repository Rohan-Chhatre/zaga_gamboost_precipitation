...
# MODEL: Meriden ~ Meriden (lags<=6) + Groton (current + lags<=6)
...
# Groton station file + NARR (USW00014707)
...
outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_meriden_groton"
...
setnames(narr_h_G, c("u","v","dew","rh","slp"),
         c("u_G","v_G","dew_G_narr","rh_G_narr","slp_G_narr"))

DT <- DT[narr_h_G, on = "DATE"]

DT[is.na(WindE_G) & !is.na(u_G),        WindE_G := u_G]
DT[is.na(WindN_G) & !is.na(v_G),        WindN_G := v_G]
DT[is.na(dew_G)  & !is.na(dew_G_narr),  dew_G  := dew_G_narr]
DT[is.na(RH_G)   & !is.na(rh_G_narr),   RH_G   := rh_G_narr]
DT[is.na(slp_G)  & !is.na(slp_G_narr),  slp_G  := slp_G_narr]

DT[, c("u_G","v_G","dew_G_narr","rh_G_narr","slp_G_narr") := NULL]
...
# log1p on rainfall lags (both stations) + OPTIONAL current Groton rain
for (i in 1:L) {
  DT[, paste0("rain_M_lag", i) := log1p(get(paste0("rain_M_lag", i)))]
  DT[, paste0("rain_G_lag", i) := log1p(get(paste0("rain_G_lag", i)))]
}
DT[, rain_G_log1p := log1p(rain_G)]  # current-time Groton rain (log1p)
...
# mu: keep your Meriden terms + add Groton rain (current + lags)
rhs_mu_gb <- paste(
  c(
    bbs_term("doy"),
    ...
    # --- current-time Groton rain (log1p) + Groton met terms ---
    bbs_term("rain_G_log1p"),
    bbs_term("RH_G"),
    bbs_term("dew_G"),
    bbs_term("slp_G"),
    bbs_term("WindE_G"),
    ...
    vapply(paste0("rain_G_lag", 2:L), bbs_term, character(1)),
...
rhs_sigma_gb <- paste(
  c(
    ...
    # --- Groton lagged terms ---
    vapply(paste0("rain_G_lag", 1:L_sigma), bbs_term, character(1)),
    vapply(paste0("RH_G_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_G_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_G_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_G_lag", 1:L), bbs_term, character(1)),
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_meriden_groton.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Meriden with Groton exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_meriden_groton.pdf"), width = 12, height = 6)
print(p)
dev.off()
