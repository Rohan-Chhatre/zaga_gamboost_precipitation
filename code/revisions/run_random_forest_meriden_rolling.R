...
# Paths
# -----------------------------
path_M <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/concatenated_files/USW00054788_merged_imputed_2004_2024.csv"

outdir <- "/home/fbs24003/rlibs/Precip_project/github/output/random_forest/rolling_validation/meriden_model1"
...
results <- foreach(
  ...
  tryCatch({
    ...
    # Fit Random Forest
    # -----------------------------
    x_cols <- c("doy", paste0("rain_M_lag", 1:L))
    
    dtr_rf <- as.data.frame(train_dt[, ..x_cols])
    dte_rf <- as.data.frame(test_dt[, ..x_cols])
    ...
    pred_log <- as.numeric(
      predict(fit.rf, newdata = dte_rf)
    )
    
    pred_te <- pmax(expm1(pred_log), 0)
    
    # -----------------------------
    # Empirical prediction intervals
    # -----------------------------
    train_pred_log <- as.numeric(
      predict(fit.rf, newdata = dtr_rf)
    )
...
if (nrow(metrics) == 0) {
  ...
  stop("All Random Forest fits failed. Check errors.csv.")
}
...
bucket_summary <- predictions[
  ...
  .(
    ...
    mae = mae(actual, rf),
    bias = bias(actual, rf),
    qmape = qmape(actual, rf),
    smape = smape(actual, rf),
    naive_mae = mae(actual, naive),
    mase = mae(actual, rf) / mae(actual, naive)
  ),
  by = rainfall_bucket
]
