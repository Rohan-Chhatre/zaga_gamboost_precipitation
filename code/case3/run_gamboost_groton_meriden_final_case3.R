...
# MODEL: Groton ~ Groton (lags<=6) + Meriden (current + lags<=6)
...
outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_groton_meriden"
...
setnames(narr_h_M, c("u","v","dew","rh","slp"),
         c("u_M","v_M","dew_M_narr","rh_M_narr","slp_M_narr"))

DT <- DT[narr_h_M, on = "DATE"]

DT[is.na(WindE_M) & !is.na(u_M),        WindE_M := u_M]
DT[is.na(WindN_M) & !is.na(v_M),        WindN_M := v_M]
DT[is.na(dew_M)  & !is.na(dew_M_narr),  dew_M  := dew_M_narr]
DT[is.na(RH_M)   & !is.na(rh_M_narr),   RH_M   := rh_M_narr]
DT[is.na(slp_M)  & !is.na(slp_M_narr),  slp_M  := slp_M_narr]

DT[, c("u_M","v_M","dew_M_narr","rh_M_narr","slp_M_narr") := NULL]
...
rhs_mu_gb <- paste(
  c(
    ...
    # --- current-time Meriden rain (log1p) + met terms ---
    bbs_term("rain_M_log1p"),
    bbs_term("RH_M"),
    bbs_term("dew_M"),
    bbs_term("slp_M"),
    bbs_term("WindE_M"),
    ...
    vapply(paste0("rain_M_lag", 2:L), bbs_term, character(1)),
...
rhs_sigma_gb <- paste(
  c(
    ...
    # --- Meriden lagged terms ---
    vapply(paste0("rain_M_lag", 1:L_sigma), bbs_term, character(1)),
    vapply(paste0("RH_M_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_M_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_M_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_M_lag", 1:L), bbs_term, character(1)),
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_groton_meriden.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Groton with Meriden exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_groton_meriden.pdf"), width = 12, height = 6)
print(p)
dev.off()
