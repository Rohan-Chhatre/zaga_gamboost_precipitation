...
# MODEL: Groton ~ Groton (lags<=6) + Danbury (current + lags<=6)
...
outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/out_zaga_gamboostlss_groton_danbury"
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_groton_danbury.rds"))
...
# -----------------------------
# Metrics by multiple peak quantiles
# -----------------------------
rmse_tr <- rmse(Y_tr, pred_tr)
rmse_te <- rmse(Y_te, pred_te)
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Groton with Danbury exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_groton_danbury.pdf"), width = 12, height = 6)
print(p)
dev.off()
