...
# MODEL: Groton ~ Groton (lags<=6) + New Haven (current + lags<=6)
...
# New Haven station file + NARR (USW00014758)
path_N   <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/concatenated_files/merged_imputed_final/USW00014758_merged_imputed_2004_2024.csv"
narr_N   <- "/home/fbs24003/rlibs/Precip_project/narr_reanalysis_data/USW00014758.csv"

outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_groton_newhaven"
...
datN <- fread(path_N, select = cols_needed)

datG[, DATE := as.POSIXct(DATE, tz = "UTC")]
datN[, DATE := as.POSIXct(DATE, tz = "UTC")]

datG <- datG[DATE >= as.POSIXct("2004-01-01 00:00:00", tz = "UTC")]
datN <- datN[DATE >= as.POSIXct("2004-01-01 00:00:00", tz = "UTC")]
...
# New Haven core vars
# -----------------------------
DT[, rain_N := numify(HourlyPrecipitation_N)]
DT[, RH_N   := numify(HourlyRelativeHumidity_N)]
DT[, dew_N  := numify(HourlyDewPointTemperature_N)]
DT[, slp_N  := numify(HourlySeaLevelPressure_N)]

DT[, `:=`(
  alpha_tilde_N = 0,
  S_h_N     = numify(HourlyWindSpeed_N),
  theta_h_N = numify(HourlyWindDirection_N)
)]
DT[, rad_N := ((alpha_tilde_N + 630 - theta_h_N) %% 360) * pi/180]
DT[, `:=`(
  WindE_N = S_h_N * cos(rad_N),
  WindN_N = S_h_N * sin(rad_N)
)]
...
# --------------------------------
# NARR imputation (New Haven)
# --------------------------------
narr_h_N <- narr_to_hourly(narr_N)
setnames(narr_h_N, c("u","v","dew","rh","slp"),
         c("u_N","v_N","dew_N_narr","rh_N_narr","slp_N_narr"))

DT <- DT[narr_h_N, on = "DATE"]

DT[is.na(WindE_N) & !is.na(u_N),        WindE_N := u_N]
DT[is.na(WindN_N) & !is.na(v_N),        WindN_N := v_N]
DT[is.na(dew_N)  & !is.na(dew_N_narr),  dew_N  := dew_N_narr]
DT[is.na(RH_N)   & !is.na(rh_N_narr),   RH_N   := rh_N_narr]
DT[is.na(slp_N)  & !is.na(slp_N_narr),  slp_N  := slp_N_narr]

DT[, c("u_N","v_N","dew_N_narr","rh_N_narr","slp_N_narr") := NULL]
...
DT[, `:=`(
  ...
  dew_WindE_N = dew_N * WindE_N,
  dew_WindN_N = dew_N * WindN_N
)]

# --------------------------------
# Lags (Groton + New Haven)
# --------------------------------
for (i in 1:L) {
  # Groton lags
  ...
  # New Haven lags
  DT[, paste0("rain_N_lag",    i) := shift(rain_N,    n = i, type = "lag")]
  DT[, paste0("RH_N_lag",      i) := shift(RH_N,      n = i, type = "lag")]
  DT[, paste0("dew_N_lag",     i) := shift(dew_N,     n = i, type = "lag")]
  DT[, paste0("slp_N_lag",     i) := shift(slp_N,     n = i, type = "lag")]
  DT[, paste0("WindE_N_lag",   i) := shift(WindE_N,   n = i, type = "lag")]
  DT[, paste0("WindN_N_lag",   i) := shift(WindN_N,   n = i, type = "lag")]
  DT[, paste0("dew_WindE_N_lag", i) := shift(dew_WindE_N, n = i, type = "lag")]
  DT[, paste0("dew_WindN_N_lag", i) := shift(dew_WindN_N, n = i, type = "lag")]
}

# log1p on rainfall lags (both stations) + OPTIONAL current New Haven rain
for (i in 1:L) {
  DT[, paste0("rain_G_lag", i) := log1p(get(paste0("rain_G_lag", i)))]
  DT[, paste0("rain_N_lag", i) := log1p(get(paste0("rain_N_lag", i)))]
}
DT[, rain_N_log1p := log1p(rain_N)]
...
vars <- c(
  ...
  # New Haven (current + lags)
  "rain_N_log1p",
  paste0("rain_N_lag", 1:L),
  "RH_N", paste0("RH_N_lag", 1:L),
  "dew_N", paste0("dew_N_lag", 1:L),
  "slp_N", paste0("slp_N_lag", 1:L),
  "WindE_N", paste0("WindE_N_lag", 1:L),
  "WindN_N", paste0("WindN_N_lag", 1:L),
  "dew_WindE_N", paste0("dew_WindE_N_lag", 1:L),
  "dew_WindN_N", paste0("dew_WindN_N_lag", 1:L)
)
...
rhs_mu_gb <- paste(
  c(
    ...
    # --- current-time New Haven rain (log1p) + met terms ---
    bbs_term("rain_N_log1p"),
    bbs_term("RH_N"),
    bbs_term("dew_N"),
    bbs_term("slp_N"),
    bbs_term("WindE_N"),
    bbs_term("WindN_N"),
    bbs_term("dew_WindE_N"),
    bbs_term("dew_WindN_N"),
    ...
    # --- New Haven lag terms ---
    vapply(paste0("rain_N_lag", 1:1), bols_term_rain, character(1)),
    vapply(paste0("rain_N_lag", 2:L), bbs_term, character(1)),
    vapply(paste0("RH_N_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindN_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindE_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("dew_WindN_N_lag", 1:L), bbs_term, character(1))
  ),
  collapse = " + "
)
...
rhs_sigma_gb <- paste(
  c(
    ...
    # --- New Haven lagged terms ---
    vapply(paste0("rain_N_lag", 1:L_sigma), bbs_term, character(1)),
    vapply(paste0("RH_N_lag",  1:L), bbs_term, character(1)),
    vapply(paste0("dew_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("slp_N_lag", 1:L), bbs_term, character(1)),
    vapply(paste0("WindE_N_lag", 1:L), bbs_term, character(1)),
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_groton_newhaven.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Groton with New Haven exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_groton_newhaven.pdf"), width = 12, height = 6)
print(p)
dev.off()
