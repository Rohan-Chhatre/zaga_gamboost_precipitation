...
# MODEL: Groton ~ Groton (lags<=6) + Sikorsky (current + lags<=6)
...
outdir <- "/home/fbs24003/rlibs/Precip_project/nalini_ct_airport_met/gam_lss paper/out_zaga_gamboostlss_groton_sikorsky"
...
saveRDS(fit.gb, file = file.path(outdir, "fit_zaga_gamboostlss_groton_sikorsky.rds"))
...
p <- ggplot(plot_df, aes(x = DATE)) +
  ...
  labs(
    title = "gamboostLSS ZAGA — Groton with Sikorsky exogenous rain (current + lags)",
    subtitle = sprintf("Test RMSE = %.4f | Peak-RMSE(95%%) = %.4f", rmse_te, peaks_te_095$rmse),
    x = "Date", y = "Hourly precipitation", color = ""
  ) +
  theme_minimal(base_size = 12)

pdf(file.path(outdir, "actual_vs_zaga_groton_sikorsky.pdf"), width = 12, height = 6)
print(p)
dev.off()
